  ...
  <div id="author_comment_list">
    ...
    <div class="author_list">
      <div
        ...
        :key="index"
      >
        <div v-if="item.authoredid == 0">{{ item.essay.title }} {{item.content}}</div>
        <div v-else>{{item.essay.title}} 回复 @{{item.authored.authorname}} : {{item.content}}</div>
      </div>
    </div>
...
.author_comment_list_item div:nth-child(1){

  font-size: 14px;
}
.author_comment_list_item div:nth-child(2){
  display: block;
