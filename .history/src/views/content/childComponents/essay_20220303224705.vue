...
export default {
  ...
  methods: {
    const authorid = JSON.parse(window.sessionStorage.getItem("author_id"));

    getEssayByEssayId(){
      this.$api
        .getEssayByEssayId(this.essayid)
        .then((res) => {
  ...
  mounted() {
    this.essayid=JSON.parse(window.sessionStorage.getItem("essay_id"));
  },
};
</script>
