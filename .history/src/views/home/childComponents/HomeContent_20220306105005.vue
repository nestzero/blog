  ...
  <div id="home-content">
    <div class="essay">
      ...
      <div class="essay-content" v-for="(item, index) in essay" :key="index">
        ...
        <div class="essay-title" @click="toEssay(item.essayid)">{{ item.title }}</div>
        <div class="essay-sentence">{{ item.sentence | ellipsisSentence }}</div>

        <div class="essay-footer">
          <span class="essay-author-name" @click="toAuthor(item.author.authorid)">{{ item.author.authorname }}</span>
...
export default {
  ...
  methods: {
    ...
    changeRank(index) {
      ...
      if(index === 2){
        this.getEssayByStars();
      }else if(index ===4){
        this.getEssayByComment();
      }
